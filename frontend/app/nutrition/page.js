...
export default function NutritionPage() {
  ...
  // ✅ Read ?tab= from URL and set active tab on mount
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      if (activeTab === 'Recipes') {
        const data = await nutritionAPI.getRecipes({
          goal: selectedGoal,
        ...
        const data = await nutritionAPI.getMealPlan({
          goal: selectedGoal,
          restrictions: restrictions.join(','),
          days: 7,
        });
        ...
        const data = await nutritionAPI.getHealthySwaps({
  ...
  return (
    <main className="min-h-screen bg-[#F8F9FC] text-[#1F2937]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        {/* Goal + Dietary Restrictions — shown on Recipes & Meal Plan */}
          ...
          <div className="space-y-6 mb-8">
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#1F2937] mb-5">
                ...
                <span className="text-[#9CA3AF] normal-case font-medium tracking-normal text-xs">(optional)</span>
              </h3>
        ...
        {/* Craving selector — Healthy Swaps only */}
        {activeTab === 'Healthy Swaps' && (
        ...
        <motion.button
          ...
          disabled={loading}
          className="w-full bg-gradient-to-r from-[#FB923C] to-[#F97316] text-white py-6 rounded-2xl font-bold uppercase tracking-[0.2em] shadow-[0_8px_20px_rgba(249,115,22,0.3)] hover:shadow-[0_12px_24px_rgba(249,115,22,0.4)] disabled:opacity-60 mb-10 flex items-center justify-center gap-3"
        >
          {loading ? (
            <>
        ...
        <AnimatePresence mode="wait">
          {generated && (
            <motion.div
              key={activeTab + selectedGoal + restrictions.join() + selectedCraving}
            ...
            >
              {activeTab === 'Recipes' && (
                recipes.length > 0
                  ...
                  : <EmptyState message="No recipes returned. Try again." />
              )}

              {activeTab === 'Meal Plan' && (
                mealPlan.length > 0
                  ? mealPlan.map((day, i) => <MealPlanCard key={i} day={day} />)
                  : <EmptyState message="No meal plan returned. Try again." />
              )}

              {activeTab === 'Healthy Swaps' && (
                swaps.length > 0
                  ? swaps.map((s, i) => <SwapCard key={i} swap={s} />)
                  : <EmptyState message="No swaps returned. Try again." />
              )}
            </motion.div>
          )}
        </AnimatePresence>
